...
class B3mB4m(object):
	def __init__(self, name=None, cve=None):
		if name != None:
			if cve != None:
				self.url = 'http://0day.today/search?search_request=%s&search_type=1&category=-1&platform=-1&price_from=0&price_to=10000&author_login=&cve=%s' % (str(name), str(cve))
			else:
				self.url = 'http://0day.today/search?search_request=%s&search_type=1&category=-1&platform=-1&price_from=0&price_to=10000&author_login=&cve=' % (str(name))
	def check(self):
